...
function renderAgentHeader(){const a=selected();$('#agent-heading').innerHTML=`${avatar(a,isMainSapi(a)?'large main-sapi-avatar':'large')}<div><h2 title="${esc(a.role)}">${esc(a.name)}</h2><p class="agent-role">${esc(a.role)}</p></div><button class="icon-button" data-action="agent-settings" aria-label="Agent settings">···</button>`;$('#message-input').placeholder=`Message ${a.name}…`;$$('[data-panel]').forEach(b=>{b.classList.toggle('active',b.dataset.panel===state.panel);b.setAttribute('aria-pressed',b.dataset.panel===state.panel);});}
...
function renderConversation(){const host=$('#conversation-body');$('#composer-area').hidden=state.panel!=='chat';renderAgentHeader();renderGlobal();
  if(state.panel==='chat'){const a=selected();const messages=getMessages(a.id);host.innerHTML=`<div class="day-divider">SAMPLE WORKSPACE</div>`+messages.map(m=>`<div class="message ${m.role==='user'?'user':''}"><div class="message-meta">${m.role==='assistant'?avatar(a,'mini'):'<span>↗</span>'}<strong>${m.role==='user'?'You':mention(a.id)}</strong><time>${esc(m.time)}</time></div><div class="message-bubble">${messageContent(m)}${m.execution?executionCard():''}${m.artifact?'<button class="artifact-link" data-action="open-brief"><span class="file-icon">▤</span><span><strong>Brightside · Kickoff brief</strong><small>DOCUMENT · JUST UPDATED</small></span><span>↗</span></button>':''}</div>${m.artifact?'<div class="message-foot">✓ Context saved to the workspace</div>':''}</div>`).join('')+(pending.has(a.id)?`<div class="typing" role="status">${esc(a.name)} is working through it ···</div>`:'');renderAttachment();}
